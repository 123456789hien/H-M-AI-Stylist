    ...
    try:
        ...
        if len(product_names) == 0:
            ...
            if len(recommendations) == 0:
                st.warning(f"No similar products found. Try selecting a different product.")
        ...
        st.subheader("📐 Smart Matching Algorithm")
        st.markdown("""
        **Strict Similarity Matching (98-100%):**
        - **Product Group Match (35%)** - MUST be same type (Garment Upper Body, Garment Lower Body, etc.)
        - **Emotion Match (35%)** - MUST be same emotion
        - **Price Range (15%)** - Similar price point (plus or minus 50% tolerance)
        - **Hotness Score (15%)** - Similar trending score
        
        **Results:**
        - Only shows products with 85%+ similarity match
        - Ensures recommendations are truly similar
        - Filters by product group first (ao only recommends ao, quan recommends quan)
        - Emotion-based clustering for relevance
        """)
